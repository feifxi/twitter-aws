...
const createPublicRoomMessage = `-- name: CreatePublicRoomMessage :one
INSERT INTO public_room_messages (room_key, sender_id, content)
VALUES ($1, $2, $3)
RETURNING id, room_key, sender_id, content, created_at
`

type CreatePublicRoomMessageParams struct {
	RoomKey  string `json:"room_key"`
	SenderID int64  `json:"sender_id"`
	Content  string `json:"content"`
}

func (q *Queries) CreatePublicRoomMessage(ctx context.Context, arg CreatePublicRoomMessageParams) (PublicRoomMessage, error) {
	row := q.db.QueryRow(ctx, createPublicRoomMessage, arg.RoomKey, arg.SenderID, arg.Content)
	var i PublicRoomMessage
	err := row.Scan(
		&i.ID,
		&i.RoomKey,
		&i.SenderID,
...
const listPublicRoomMessages = `-- name: ListPublicRoomMessages :many
SELECT id, room_key, sender_id, content, created_at
...
type ListPublicRoomMessagesParams struct {
	RoomKey string `json:"room_key"`
	Limit   int32  `json:"limit"`
	Offset  int32  `json:"offset"`
}

func (q *Queries) ListPublicRoomMessages(ctx context.Context, arg ListPublicRoomMessagesParams) ([]PublicRoomMessage, error) {
	rows, err := q.db.Query(ctx, listPublicRoomMessages, arg.RoomKey, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []PublicRoomMessage{}
	for rows.Next() {
		var i PublicRoomMessage
		if err := rows.Scan(
			&i.ID,
